...
func TestCrawlerStruct_Get(t *testing.T) {
	startTime := time.Now().UTC()
	jobs := make(chan WebInfo, 10)
	findResult := make(chan FindResult, 10)
	go createJobs(jobs)

	var wg sync.WaitGroup
	var workerCount = 20 //定义工作协程数量
	for i := 1; i <= workerCount; i++ {
		wg.Add(1)
		go worker(i, jobs, findResult, &wg)
	}
	wg.Wait()
	close(findResult)
	fmt.Printf("总耗时：%s\n", time.Now().UTC().Sub(startTime))
	for result := range findResult {
		fmt.Printf("%s平台,查找%s结果为:%v", result.PlatformName, result.FindContent, result.IsFind)
	}
}

func createJobs(jobs chan<- WebInfo) {
	list := []UrlInfo{
	...
	urlInfoChan := make(chan UrlInfo, len(list))
	for _, urlInfo := range list {
		urlInfoChan <- urlInfo
	}
	close(urlInfoChan)
	...
	for i := 1; i <= workerCount; i++ {
		...
		go func() {
			urlInfo := <-urlInfoChan
			httpStatus, content, err := Fetcher(urlInfo.Url, 10)
			if err != nil {
				webInfo := WebInfo{PlatformName: urlInfo.PlatformName, PlatformUrl: urlInfo.PlatformUrl, FindContent: urlInfo.FindContent, HttpStatus: httpStatus, Content: content}
				jobs <- webInfo
			}
			wg.Done()
		}()
...
func worker(id int, jobs <-chan WebInfo, findResult chan<- FindResult, wg *sync.WaitGroup) {
	fmt.Printf("worker %d starting\n", id)
	defer wg.Done()
	for job := range jobs {
		//检验状态
		if job.HttpStatus != http.StatusOK {
			result := FindResult{PlatformName: job.PlatformName, PlatformUrl: job.PlatformName, FindContent: job.FindContent, IsFind: false}
			findResult <- result
			return
		}
		//查找内容
		re := regexp.MustCompile(job.FindContent)
		findStr := re.FindString(string(job.Content))
		if len(findStr) > 0 {
			result := FindResult{PlatformName: job.PlatformName, PlatformUrl: job.PlatformName, FindContent: job.FindContent, IsFind: true}
			findResult <- result
			return
		}
		result := FindResult{PlatformName: job.PlatformName, PlatformUrl: job.PlatformName, FindContent: job.FindContent, IsFind: false}
